...
//   답변왕: 짧지 않은 댓글 40개 이상 — 왕관 + 빨강→주황 반짝이 글자
//   채택왕: 채택(고정)된 댓글 5개 이상 — 금메달 + 골드 반짝이 글자
//   활동왕: 주간 경험치 100 이상 — 새싹 + 그린 반짝이 글자
...
export default function KingBadges({ answer, pick, activity }: Props) {
  ...
  return (
    <>
      {answer && <Badge kind="answer" icon="/icons/answer-king-crown.svg" label="답변왕" title="최근 7일 댓글 40개 이상" />}
      {pick && <Badge kind="pick" icon="/icons/toss/medal-gold.svg" label="채택왕" title="최근 7일 채택된 댓글 5개 이상" />}
      {activity && <Badge kind="activity" icon="/icons/sprout.svg" label="활동왕" title="최근 7일 경험치 100 이상" />}
    </>
  );
}
