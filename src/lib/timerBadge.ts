// 타이머 불꽃 등급 8단계. 누적 공부시간(초)으로 결정된다.
//
...
// "불이 점점 뜨거워지는" 순서다.
//
// 임계값은 실제 분포에 맞췄다(2026-09-14 기준 타이머 사용자 334명 중 1시간 이상 42명,
// 10시간 이상 25명, 30시간 이상 7명, 50시간 이상 2명). 너무 촘촘하면 다 같은 등급이 된다.
export interface FlameTier {
  id: string;
  name: string;
  /** 이 등급이 되는 최소 누적 공부시간(초) */
  minSeconds: number;
  icon: string;
  /** 뱃지 글자/테두리 색 */
  color: string;
...
export const FLAME_TIERS: FlameTier[] = [
  ...
  { id: "campfire",  name: "모닥불", minSeconds: 5 * H,   icon: "/icons/flame/f3.svg", color: "#E8541A", soft: "#FFEDE4" },
  { id: "bonfire",   name: "화톳불", minSeconds: 10 * H,  icon: "/icons/flame/f4.svg", color: "#E0203F", soft: "#FFE9EC" },
  { id: "torch",     name: "횃불",   minSeconds: 25 * H,  icon: "/icons/flame/f5.svg", color: "#C82A78", soft: "#FFE8F3" },
  { id: "wildfire",  name: "들불",   minSeconds: 50 * H,  icon: "/icons/flame/f6.svg", color: "#7248E0", soft: "#F0EAFF" },
  { id: "pillar",    name: "불기둥", minSeconds: 100 * H, icon: "/icons/flame/f7.svg", color: "#1E6BE0", soft: "#E6F0FF" },
  { id: "phoenix",   name: "불사조", minSeconds: 200 * H, icon: "/icons/flame/f8.svg", color: "#C98A00", soft: "#FFF6DF" },
];

/** 누적 공부시간(초)에 해당하는 등급. 항상 하나는 나온다(0초 = 불씨). */
