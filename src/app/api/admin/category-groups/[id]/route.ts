...
import {
  deleteOrDeactivateCategoryGroup,
  getCategoryGroups,
  mapGroup,
  normalizeSlug,
  updateCategoryGroup,
...
function adminError(error: unknown) {
  ...
  console.error("Admin category group detail API error:", error);
...
export async function DELETE(
  ...
  try {
    await requireAdmin();
    const { id } = await params;
    const result = await deleteOrDeactivateCategoryGroup(id);
    const groups = await getCategoryGroups({ activeOnly: false });
    return NextResponse.json({ ...result, groups: groups.map(mapGroup) });
  } catch (error) {
    return adminError(error);
  }
}
