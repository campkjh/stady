...
export default async function NoticePage({
  ...
  const items = notices.map((notice) => {
    ...
    header: (
      <div>
        <p style={{ fontSize: 15, fontWeight: 600, color: "var(--c-text-c)", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
          {notice.isRecent && (
        ...
        {notice.dateLabel && (
          <p style={{ fontSize: 13, color: "var(--c-text-4c)" }}>{notice.dateLabel}</p>
        )}
      </div>
    ),
    content: (
      <div style={{ padding: "0 0 16px" }}>
        <div style={{ fontSize: 14, lineHeight: 1.6, color: "var(--c-text-2d)", whiteSpace: "pre-wrap" }}>
          {notice.body}
        </div>
